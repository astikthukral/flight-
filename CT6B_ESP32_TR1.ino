...
#define SERIAL_CAL_WINDOW_MS 5000  // accept "CAL" command within 5s after boot
...
bool checkSerialCalibrationCommand(uint32_t windowMs) {
  uint32_t start = millis();
  Serial.println("Type 'CAL' within 5s to enter ESC calibration mode...");
  while (millis() - start < windowMs) {
    if (Serial.available()) {
      String s = Serial.readStringUntil('\n');
      s.trim();
      s.toUpperCase();
      if (s == "CAL") return true;
    }
    delay(5);
  }
  return false;
...
void setup() {
  ...
  Serial.println("ESP32 Hexacopter (CH3 -> 6 ESC @50Hz) with ESC Calibration Mode");

  // Decide calibration mode: jumper or serial command
  bool jumper = checkCalibrationJumper();
  bool serialCmd = false;
  if (!jumper) {
    serialCmd = checkSerialCalibrationCommand(SERIAL_CAL_WINDOW_MS);
  }
  calibrationRequested = jumper || serialCmd;
